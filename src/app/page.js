...
<div className="d-flex justify-content-center">    <button
  ...
  const renderReward = () => (
    <div className="content-card p-4 p-sm-5 d-flex flex-column align-items-center text-center reward-container">
      
      {/* CONGRATS Text Art */}
      <div className="mb-4 reward-visual">
        <img 
            src="https://i.imgur.com/gK9qQ4K.png" 
            alt="Congratulations" 
            className="congrats-img"
            onError={(e) => { e.target.onerror = null; e.target.src = 'https://placehold.co/300x150/ff4d4d/ffffff?text=CONGRATS' }}
        />
      </div>
      
      {userData.prize && userData.prize.label !== "Better Luck Next Time" ? (
        <>
          <p className="fs-4 fw-bold text-dark mb-4">
            You won {userData.prize.label}!
          </p>
          <div className="bg-white bg-opacity-75 p-3 rounded-3 w-100 mb-4 border border-danger">
            <p className="small text-secondary mb-1">Registration ID: <span className="fw-semibold">{userData.registrationId}</span></p>
            <input 
                type="text" 
                value={userData.code} 
                readOnly 
                className="form-control text-center fs-5 fw-bold text-danger bg-white border-danger"
            />
          </div>
          <p className="small text-dark mb-4 opacity-75">
        ...
        <>
          <p className="fs-4 fw-bold text-dark mb-4">
            {userData.prize?.label || "Better luck next time!"}
          </p>
          <p className="small text-dark mb-4 opacity-75">
      ...
      <button
        onClick={() => {
          setUserData({ name: '', mobile: '', city: '', registrationId: '', prize: null, code: null });
          setFormData({ name: '', mobile: '', city: '' });
          setCurrentView('register');
        }}
        className="btn btn-light btn-lg w-100 fw-bold btn-reset-custom"
      >
        Play Again
      </button>
    </div>
...
.bottom-decoration {
  ...
  pointer-events: none; /* prevents blocking button clicks */
        ...
        //     width: 100%;
        //     max-width: 380px;
        //     color: #333; /* Ensure dark text on light glass background */
        // }
.content-card {
